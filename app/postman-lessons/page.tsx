    ...
    tasks: [
      {
        ...
        challenge: "Изучите интерфейс Postman: создайте новый запрос, настройте метод и URL, отправьте запрос и изучите ответ.",
        beforeImage: "/screenshots/postman/basics-before.svg",
        afterImage: "/screenshots/postman/basics-after.svg",
        solution: {
          steps: [
            "1. Откройте Postman (встроенный инструмент на сайте)",
            "2. Создайте новый запрос:",
            "  • Нажмите 'New' → 'Request'",
            "  • Введите название: 'Test API'",
            "  • Выберите коллекцию или создайте новую",
            "3. Настройте запрос:",
            "  • Выберите метод GET",
          ...
          tips: [
            "Postman автоматически форматирует JSON ответы",
            "Используйте вкладки для организации запросов",
            "Сохраняйте часто используемые запросы в коллекции",
      ...
      },
      {
        ...
        challenge: "Создайте коллекцию 'API Testing', добавьте в неё несколько запросов и организуйте их по папкам.",
        ...
        solution: {
          steps: [
            ...
            "  • Название: 'API Testing'",
            "  • Описание: 'Коллекция для изучения API'",
            "2. Создайте папки в коллекции:",
            "  • Правая кнопка на коллекции → 'Add Folder'",
            "  • Создайте папки: 'Users', 'Posts', 'Comments'",
            "3. Добавьте запросы в папки:",
            "  • GET /users - получить список пользователей",
            "  • GET /posts - получить список постов",
            "  • GET /comments - получить список комментариев",
            "4. Организуйте запросы:",
            "  • Перетащите запросы в соответствующие папки",
            "  • Добавьте описания к каждому запросу"
          ],
          tips: [
            "Используйте папки для группировки связанных запросов",
            "Добавляйте описания для лучшего понимания",
            "Используйте осмысленные названия для запросов",
            "Организуйте коллекции по проектам или API"
          ]
        }
      },
      {
        ...
        challenge: "Создайте окружение 'Development', добавьте переменные baseUrl и apiKey, используйте их в запросах.",
        ...
        solution: {
          steps: [
            ...
            "  • Выберите 'Development' в выпадающем списке",
            "4. Используйте переменные в запросах:",
            "  • URL: {{baseUrl}}/users/{{userId}}",
            "  • Header: Authorization: Bearer {{apiKey}}",
            "5. Протестируйте запросы:",
            "  • Убедитесь, что переменные подставляются корректно"
          ],
          tips: [
            "Используйте переменные для переключения между окружениями",
            "Храните чувствительные данные в переменных",
            "Используйте осмысленные названия переменных",
            "Создавайте отдельные окружения для dev/staging/prod"
          ]
        }
      },
      {
        ...
        challenge: "Создайте POST запрос для создания нового поста и PUT запрос для обновления существующего поста.",
        ...
        solution: {
          steps: [
            ...
            "3. Добавьте тело запроса (Body):",
            "  • Выберите 'raw' и 'JSON'",
            "  • Введите JSON:",
            "  • {\"title\": \"New Post\", \"body\": \"Post content\", \"userId\": 1}",
            "4. Отправьте запрос и проверьте ответ:",
            "  • Статус: 201 Created",
            "  • В ответе должен быть ID нового поста",
            "5. Создайте PUT запрос:",
            "  • Метод: PUT",
            "  • URL: {{baseUrl}}/posts/1",
            "  • Обновите данные в Body"
          ],
          tips: [
            "POST создает новые ресурсы, PUT обновляет существующие",
            "Всегда указывайте Content-Type для JSON",
            "Проверяйте статус коды: 201 для создания, 200 для обновления",
      ...
      },
      {
        ...
        challenge: "Добавьте тесты для проверки статус кода, времени ответа и структуры данных в ваши запросы.",
        beforeImage: "/screenshots/postman/testing-before.svg",
        afterImage: "/screenshots/postman/testing-after.svg",
        solution: {
          steps: [
            "1. Откройте вкладку 'Tests' в запросе",
            "2. Добавьте тест для статус кода:",
            "  • pm.test('Status code is 200', () => {",
            "  •   pm.response.to.have.status(200);",
            "  • });",
            "3. Добавьте тест для времени ответа:",
            "  • pm.test('Response time is less than 1000ms', () => {",
            "  •   pm.expect(pm.response.responseTime).to.be.below(1000);",
            "  • });",
            "4. Добавьте тест для структуры данных:",
            "  • pm.test('Response has required fields', () => {",
            "  •   const jsonData = pm.response.json();",
            "  •   pm.expect(jsonData).to.have.property('id');",
            "  •   pm.expect(jsonData).to.have.property('title');",
            "  • });",
            "5. Запустите запрос и проверьте результаты тестов"
          ],
          tips: [
            "Используйте pm.test() для создания тестов",
            "pm.response содержит информацию об ответе",
            "pm.expect() для проверки условий",
            "Тесты выполняются автоматически после каждого запроса"
          ]
        }
      }
    ]
