        ...
        },
        {
          ...
          solution: {
            steps: [
              ...
              "2. Очистите список запросов: нажмите кнопку 🚫 (Clear)",
              "3. Откройте демо для генерации запросов:",
              "  • Нажмите кнопку 'Открыть демо Network' ниже",
              "  • На новой странице нажимайте кнопки GET/POST/PUT/DELETE",
              "  • Каждый запрос появится в Network панели",
              "4. Изучите список запросов:",
              "  • Найдите запросы к /api/devtools-demo",
              "  • Обратите внимание на статус-коды: 200 (успех), 404 (не найдено), 500 (ошибка сервера)",
              "  • Посмотрите на время загрузки справа от каждого запроса",
              "  • Сравните быстрые и медленные запросы (Slow кнопки)",
              "5. Детальный анализ запроса:",
              "  • Кликните на любой API запрос в списке",
              "  • Вкладка 'Headers': изучите заголовки запроса и ответа",
              "  • Вкладка 'Response': посмотрите JSON содержимое ответа",
              "  • Вкладка 'Timing': изучите фазы загрузки (DNS, Connect, Request, Response)",
              "6. Сравните разные методы и сценарии:",
              "  • GET: простой запрос без тела (безопасный, идемпотентный)",
              "  • POST: создание ресурса с JSON данными (не идемпотентный)",
              "  • PUT: полная замена ресурса (идемпотентный)",
              "  • PATCH: частичное обновление ресурса (может быть идемпотентным)",
              "  • DELETE: удаление ресурса (идемпотентный)",
              "  • Ошибки: 404, 500, 401, 403, 409 - изучите разные типы ошибок",
              "  • Производительность: медленные запросы (2-3 сек) и большие данные"
            ],
            tips: [
              "🌐 Network панель показывает ВСЕ сетевые запросы страницы в реальном времени",
              "✅ Зеленые запросы (200) = успешные, 🔴 красные (4xx, 5xx) = ошибки",
              "⏱️ Время загрузки показано справа от каждого запроса в миллисекундах",
              "🔍 Используйте фильтры: XHR (AJAX), JS, CSS, Img для поиска конкретных типов",
              "📊 Вкладка 'Timing' показывает детальную разбивку времени загрузки",
              "📋 Вкладка 'Headers' содержит заголовки запроса и ответа (Content-Type, Authorization и др.)",
              "📄 Вкладка 'Response' показывает содержимое ответа сервера (JSON, HTML, текст)",
              "🔄 Кнопка 'Clear' (🚫) очищает список запросов для новых экспериментов",
              "🎯 Кликните на запрос для детального просмотра всех его параметров",
              "💾 Network панель полезна для отладки API, анализа производительности и поиска ошибок"
            ]
          }
        },
        {
          id: 4,
          title: "Отладка с Breakpoint - практический урок",
          description: "Научитесь находить и исправлять ошибки в коде с помощью breakpoint и пошаговой отладки",
          category: "Sources",
          completed: false,
          challenge: "Найдите ошибку в коде: функция должна возвращать сумму чисел, но возвращает неправильный результат. Используйте breakpoint для поиска проблемы.",
          solution: {
            steps: [
              "1. Откройте DevTools (F12) и перейдите на вкладку Sources",
              "2. Создайте новый сниппет:",
              "  • В левой панели найдите 'Snippets' (Сниппеты)",
              "  • Нажмите '+' рядом с 'Snippets' для создания нового",
              "  • Назовите его 'debug-practice'",
              "3. Скопируйте код с ошибкой из демо-блока ниже",
              "4. Поставьте breakpoint на строке с 'return result':",
              "  • Кликните на номер строки слева от 'return result'",
              "  • Появится красная точка - это breakpoint",
              "5. Запустите код:",
              "  • Нажмите Ctrl+Enter в сниппете",
              "  • Код остановится на breakpoint",
              "6. Изучите переменные в панели 'Scope':",
              "  • Посмотрите значения: a = 5, b = 3, result = ?",
              "  • Найдите ошибку: result должен быть 8, но равен другому числу",
              "7. Используйте Step Over (F10) для пошагового выполнения:",
              "  • Нажмите F10 несколько раз, чтобы пройти код построчно",
              "  • Следите как изменяются переменные",
              "8. Найдите и исправьте ошибку:",
              "  • В коде есть ошибка в вычислении",
              "  • Исправьте её и запустите код снова",
              "9. Проверьте результат:",
              "  • Теперь функция должна возвращать правильную сумму"
            ],
            tips: [
              "🔍 Breakpoint - это точка остановки кода для анализа",
              "📊 Панель 'Scope' показывает все переменные и их текущие значения",
              "⏭️ Step Over (F10) - выполнить одну строку кода",
              "🔍 Step Into (F11) - войти внутрь функции",
              "▶️ Continue - продолжить выполнение до следующего breakpoint",
              "❌ Удалить breakpoint: кликните на красную точку еще раз",
              "🐛 Практическое применение: отладка обработчиков событий, валидации форм",
              "📝 В баг-репорте указывайте: 'На строке X переменная Y имеет неожиданное значение Z'",
              "🎯 Breakpoint помогает понять логику работы кода",
              "💡 Используйте несколько breakpoint для отслеживания сложной логики"
            ]
          }
        },
        {
            ...
            {currentLesson.tasks.map((task) => (
              <div key={task.id} className="bg-white rounded-lg shadow-md p-6">
                ...
                <div className="flex items-start justify-between mb-6">
                  ...
                  {task.category === 'Sources' && (
                    <div className="my-3 p-4 bg-orange-50 border border-orange-200 rounded-lg">
                      <div className="font-semibold text-orange-800 mb-2">🎯 Практическое задание: найти ошибку</div>
                      <p className="text-orange-700 mb-3">В коде ниже есть ошибка! Функция должна возвращать сумму чисел, но возвращает неправильный результат. Используйте breakpoint для поиска проблемы.</p>
                      
                      <div className="bg-red-50 border border-red-200 p-3 rounded mb-3">
                        <p className="text-red-800 text-sm">
                          <strong>🐛 Задание:</strong><br/>
                          • Функция должна вернуть 5 + 3 = 8<br/>
                          • Но она возвращает другое число<br/>
                          • Найдите ошибку с помощью breakpoint<br/>
                          • Исправьте код и проверьте результат
                        </p>
                      </div>
                      
                      <div className="bg-gray-900 text-white p-4 rounded-lg mb-3 font-mono text-sm">
                        <div className="text-gray-400 mb-2">// Скопируйте этот код в сниппет DevTools:</div>
                        <div className="text-green-300">function calculateSum(a, b) {'{'}</div>
                        <div className="ml-4 text-white">console.log('Начинаем расчет...');</div>
                        <div className="ml-4 text-red-300">const result = a * b; // ← ОШИБКА! Должно быть a + b</div>
                        <div className="ml-4 text-yellow-300">console.log('Результат:', result); // ← Поставьте breakpoint здесь</div>
                        <div className="ml-4 text-white">return result;</div>
                        <div className="text-green-300">{'}'}</div>
                        <div className="mt-2 text-blue-300">calculateSum(5, 3); // ← Вызов функции</div>
                      </div>
                      
                      <div className="bg-blue-50 border border-blue-200 p-3 rounded mb-3">
                        <p className="text-blue-800 text-sm">
                          <strong>💡 Подсказка:</strong><br/>
                          • Поставьте breakpoint на строке с 'return result'<br/>
                          • Запустите код и посмотрите значение переменной 'result'<br/>
                          • В панели 'Scope' увидите: a=5, b=3, result=15<br/>
                          • Но должно быть result=8! Найдите ошибку в вычислении
                        </p>
                      </div>
                      
                      <div className="bg-green-50 border border-green-200 p-3 rounded mb-3">
                        <p className="text-green-800 text-sm">
                          <strong>✅ Правильный код (после исправления):</strong><br/>
                          const result = a + b; // Вместо a * b
                        </p>
                      </div>
                      ...
                      <button
                        onClick={() => {
                          function calculateSum(a: number, b: number) {
                            ...
                            const result = a * b; // Ошибка!
                            console.log('Результат:', result);
                            return result;
                          }
                          const wrongResult = calculateSum(5, 3);
                          console.log('Неправильный результат:', wrongResult);
                          console.log('Должно быть:', 5 + 3);
                        }}
                        className="bg-orange-600 hover:bg-orange-700 text-white px-4 py-2 rounded-lg font-medium transition-colors mr-2"
                      >
                        🐛 Запустить код с ошибкой
                      </button>
                      ...
                      <button
                        onClick={() => {
                          function calculateSum(a: number, b: number) {
                            console.log('Начинаем расчет...');
                            const result = a + b; // Исправлено!
                            console.log('Результат:', result);
                            return result;
                          }
                          const correctResult = calculateSum(5, 3);
                          console.log('Правильный результат:', correctResult);
                        }}
                        className="bg-green-600 hover:bg-green-700 text-white px-4 py-2 rounded-lg font-medium transition-colors"
                      >
                        ✅ Запустить исправленный код
                      </button>
                    </div>
                  )}
                  </div>
                ...
                {/* Скриншоты до и после - только для заданий с визуальными изменениями */}
                {task.category !== 'Network' && (
