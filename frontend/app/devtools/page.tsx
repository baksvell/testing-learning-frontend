        ...
        },
        {
          ...
          solution: {
            steps: [
              ...
              "  • Если папки нет, она появится автоматически",
              ...
              "  • Выделите весь код из серого блока",
              "  • Скопируйте его (Ctrl+C)",
              "  • Вставьте в редактор сниппета (Ctrl+V)",
              "  • Нажмите Ctrl+S для сохранения",
              "4. Поставьте breakpoint на строке с 'return result':",
              "  • Найдите строку: 'return result;'",
              "  • Кликните на номер строки слева от неё (обычно это 5)",
              "  • Появится красная точка - это breakpoint",
              "  • Если точка не появилась, кликните еще раз",
              "5. Запустите код:",
              "  • Нажмите Ctrl+Enter в сниппете",
              "  • Или нажмите кнопку 'Run' (▶️) внизу редактора",
              "  • Код остановится на breakpoint",
              "  • Вы увидите желтую стрелку на строке с breakpoint",
              "6. Изучите переменные в панели 'Scope' (справа):",
              "  • Найдите раздел 'Local' в панели Scope",
              "  • Посмотрите значения: a = 5, b = 3, result = 15",
              "  • Обратите внимание: result = 15, но должно быть 8!",
              "  • Это означает, что в коде есть ошибка",
              "7. Используйте Step Over (F10) для пошагового выполнения:",
              "  • Нажмите F10 один раз - код перейдет к следующей строке",
              "  • Следите как изменяются переменные в панели Scope",
              "  • Попробуйте нажать F10 еще раз",
              "  • Изучите все значения переменных",
              "8. Найдите ошибку в коде:",
              "  • Посмотрите на строку: 'const result = a * b;'",
              "  • Здесь ошибка! Должно быть: 'const result = a + b;'",
              "  • a * b = 5 * 3 = 15 (неправильно)",
              "  • a + b = 5 + 3 = 8 (правильно)",
              "9. Исправьте ошибку:",
              "  • Измените строку на: 'const result = a + b;'",
              "  • Нажмите Ctrl+S для сохранения",
              "  • Удалите старый breakpoint (кликните на красную точку)",
              "  • Поставьте новый breakpoint на 'return result'",
              "10. Проверьте исправленный код:",
              "  • Нажмите Ctrl+Enter для запуска",
              "  • Код остановится на breakpoint",
              "  • В панели Scope теперь result = 8 (правильно!)",
              "  • Нажмите F10 или Continue (▶️) для завершения",
              "  • В консоли увидите: 'Результат: 8'"
            ],
            tips: [
              "🔍 Breakpoint - это точка остановки кода для анализа. Код останавливается перед выполнением строки с breakpoint",
              "📊 Панель 'Scope' показывает все переменные и их текущие значения. Раздел 'Local' содержит переменные текущей функции",
              "⏭️ Step Over (F10) - выполнить одну строку кода и перейти к следующей. Самый часто используемый способ отладки",
              "🔍 Step Into (F11) - войти внутрь функции, если текущая строка содержит вызов функции",
              "▶️ Continue (F8) - продолжить выполнение до следующего breakpoint или до конца программы",
              "❌ Удалить breakpoint: кликните на красную точку еще раз. Точка исчезнет",
              "🐛 Практическое применение: отладка обработчиков событий, валидации форм, API запросов",
              "📝 В баг-репорте указывайте: 'На строке X переменная Y имеет неожиданное значение Z вместо ожидаемого W'",
              "🎯 Breakpoint помогает понять логику работы кода и найти места, где переменные принимают неожиданные значения",
              "💡 Используйте несколько breakpoint для отслеживания сложной логики и понимания потока выполнения",
              "🔧 Если код не останавливается на breakpoint, убедитесь что строка с breakpoint действительно выполняется",
              "📋 В панели 'Call Stack' видно какие функции вызвали текущую функцию - полезно для понимания контекста",
              "⏸️ Пауза на breakpoint позволяет изучить состояние программы в конкретный момент времени",
              "🎨 Желтая стрелка показывает текущую строку выполнения, красная точка - breakpoint"
            ]
          }
        },
        {
