'use client'

import { useState } from 'react'
import { ArrowLeft, Shield, Clock, BookOpen, Users, Award, Code, Globe, Bug } from 'lucide-react'
import Link from 'next/link'

export default function SecurityPage() {
  return (
    <div className="min-h-screen bg-gray-50">
      {/* Header */}
      <div className="bg-white shadow-sm border-b">
        <div className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8">
          <div className="flex justify-between items-center py-6">
            <div className="flex items-center space-x-4">
              <Link 
                href="/"
                className="flex items-center text-gray-600 hover:text-gray-900 transition-colors"
              >
                <ArrowLeft className="w-5 h-5 mr-2" />
                Назад в главное меню
              </Link>
            </div>
            <div className="flex items-center space-x-2">
              <Shield className="w-8 h-8 text-red-500" />
              <h1 className="text-2xl font-bold text-gray-900">Безопасность</h1>
            </div>
          </div>
        </div>
      </div>

      {/* Main Content */}
      <div className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8 py-12">
        {/* Coming Soon Banner */}
        <div className="bg-gradient-to-r from-red-500 to-red-600 rounded-2xl p-8 text-white text-center mb-12">
          <div className="flex justify-center mb-4">
            <Clock className="w-16 h-16 text-white" />
          </div>
          <h2 className="text-3xl font-bold mb-4">Уроки скоро будут готовы!</h2>
          <p className="text-xl text-red-100 mb-6">
            Мы работаем над созданием интерактивных уроков по тестированию безопасности
          </p>
          <div className="bg-white/20 rounded-lg p-4 inline-block">
            <p className="text-lg font-semibold">Ожидайте в ближайшее время</p>
          </div>
        </div>

        {/* What to Expect */}
        <div className="grid md:grid-cols-2 gap-8 mb-12">
          <div className="bg-white rounded-xl p-6 shadow-lg">
            <div className="flex items-center mb-4">
              <Shield className="w-8 h-8 text-red-500 mr-3" />
              <h3 className="text-xl font-semibold text-gray-900">Тестирование безопасности</h3>
            </div>
            <ul className="space-y-3 text-gray-600">
              <li className="flex items-start">
                <span className="w-2 h-2 bg-red-500 rounded-full mt-2 mr-3 flex-shrink-0"></span>
                XSS (Cross-Site Scripting) атаки
              </li>
              <li className="flex items-start">
                <span className="w-2 h-2 bg-red-500 rounded-full mt-2 mr-3 flex-shrink-0"></span>
                SQL инъекции и защита от них
              </li>
              <li className="flex items-start">
                <span className="w-2 h-2 bg-red-500 rounded-full mt-2 mr-3 flex-shrink-0"></span>
                CSRF (Cross-Site Request Forgery)
              </li>
              <li className="flex items-start">
                <span className="w-2 h-2 bg-red-500 rounded-full mt-2 mr-3 flex-shrink-0"></span>
                Аутентификация и авторизация
              </li>
              <li className="flex items-start">
                <span className="w-2 h-2 bg-red-500 rounded-full mt-2 mr-3 flex-shrink-0"></span>
                Тестирование HTTPS и SSL
              </li>
            </ul>
          </div>

          <div className="bg-white rounded-xl p-6 shadow-lg">
            <div className="flex items-center mb-4">
              <BookOpen className="w-8 h-8 text-blue-500 mr-3" />
              <h3 className="text-xl font-semibold text-gray-900">Интерактивные задания</h3>
            </div>
            <ul className="space-y-3 text-gray-600">
              <li className="flex items-start">
                <span className="w-2 h-2 bg-blue-500 rounded-full mt-2 mr-3 flex-shrink-0"></span>
                Практические примеры уязвимостей
              </li>
              <li className="flex items-start">
                <span className="w-2 h-2 bg-blue-500 rounded-full mt-2 mr-3 flex-shrink-0"></span>
                Инструменты для тестирования безопасности
              </li>
              <li className="flex items-start">
                <span className="w-2 h-2 bg-blue-500 rounded-full mt-2 mr-3 flex-shrink-0"></span>
                Анализ кода на уязвимости
              </li>
              <li className="flex items-start">
                <span className="w-2 h-2 bg-blue-500 rounded-full mt-2 mr-3 flex-shrink-0"></span>
                Тестирование веб-приложений
              </li>
              <li className="flex items-start">
                <span className="w-2 h-2 bg-blue-500 rounded-full mt-2 mr-3 flex-shrink-0"></span>
                Отчеты по безопасности
              </li>
            </ul>
          </div>
        </div>

        {/* Progress Indicator */}
        <div className="bg-white rounded-xl p-6 shadow-lg">
          <div className="flex items-center justify-between mb-4">
            <h3 className="text-xl font-semibold text-gray-900">Прогресс разработки</h3>
            <span className="text-sm text-gray-500">В разработке</span>
          </div>
          <div className="w-full bg-gray-200 rounded-full h-3 mb-4">
            <div className="bg-red-500 h-3 rounded-full" style={{ width: '25%' }}></div>
          </div>
          <p className="text-gray-600">
            Мы активно работаем над созданием качественных уроков по тестированию безопасности. 
            Следите за обновлениями!
          </p>
        </div>

        {/* Alternative Learning */}
        <div className="mt-12 text-center">
          <h3 className="text-2xl font-bold text-gray-900 mb-6">Пока изучайте другие темы</h3>
          <div className="grid grid-cols-2 md:grid-cols-4 gap-4">
            <Link href="/devtools" className="bg-indigo-500 text-white p-4 rounded-lg hover:bg-indigo-600 transition-colors">
              <Code className="w-8 h-8 mx-auto mb-2" />
              <p className="font-semibold">DevTools</p>
            </Link>
            <Link href="/api" className="bg-green-500 text-white p-4 rounded-lg hover:bg-green-600 transition-colors">
              <Globe className="w-8 h-8 mx-auto mb-2" />
              <p className="font-semibold">API</p>
            </Link>
            <Link href="/functional" className="bg-yellow-500 text-white p-4 rounded-lg hover:bg-yellow-600 transition-colors">
              <Bug className="w-8 h-8 mx-auto mb-2" />
              <p className="font-semibold">Функциональное</p>
            </Link>
            <Link href="/ui-ux" className="bg-purple-500 text-white p-4 rounded-lg hover:bg-purple-600 transition-colors">
              <BookOpen className="w-8 h-8 mx-auto mb-2" />
              <p className="font-semibold">UI/UX</p>
            </Link>
          </div>
        </div>
      </div>
    </div>
  )
}